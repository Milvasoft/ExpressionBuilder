...
{
    private readonly DbDataContext _db = new();
    ...
    public void FilterWithoutStatements()
    {
        ...
        var products = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => true);
    ...
    public void FilterWithSimpleStatement()
    {
        ...
        var products = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => p.Discontinued);
    ...
    public void FilterCastingTheValueToObject()
    {
        ...
        var products = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => p.UnitPrice > 100);
        Assert.That(products, Is.EquivalentTo(solution));
    }
    ...
    public void FilterWithPropertyChainFilterStatements()
    {
        ...
        filter.By("ProductID", Operation.In, _values);
        var products = _db.Products.Where(filter);
        var solution = _db.Products.Where(p =>
            (p.Categories != null && p.Categories.CategoryName != null &&
             p.Categories.CategoryName.Trim().ToLower().Equals("beverages")) || _values.Contains(p.ProductID));
        Assert.That(products, Is.EquivalentTo(solution));
    }
    ...
    public void FilterWithPropertyListFilterStatements()
    {
        ...
        var people = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => p.OrderDetails != null && p.OrderDetails.Any(o => o.Discount > 0F) &&
                                              (p.Categories != null && p.Categories.CategoryName != null &&
    ...
    public void FilterWithFilterStatementWithListOfValues()
    {
        ...
        filter.By("ProductID", Operation.In, _values);
        var people = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => _values.Contains(p.ProductID));
        Assert.That(people, Is.EquivalentTo(solution));
    }
    ...
    public void BuilderWithSingleFilterStatementWithBetween()
    {
        ...
        var people = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => p.ProductID >= 2 && p.ProductID <= 5);
        Assert.That(people, Is.EquivalentTo(solution));
    }
    ...
    public void BuilderUsingIsNullOperation()
    {
        ...
        var people = _db.Orders.Where(filter);
        var solution = _db.Orders.Where(p => p.ShipRegion == null);
        Assert.That(people, Is.EquivalentTo(solution));
    }
    ...
    public void BuilderUsingComplexExpressionsFluentInterface()
    {
        ...
        var people = _db.Products.Where(filter);
        var solution = _db.Products.Where(p => p.SupplierID == 1 && (p.CategoryID == 1 || p.CategoryID == 2));

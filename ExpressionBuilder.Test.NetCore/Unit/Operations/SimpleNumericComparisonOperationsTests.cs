...
public class SimpleNumericComparisonOperationsTests
{
    ...
    public void GetExpressionTest(ExpressionType method, string methodName, double value)
    {
        ...
        var solutionMethod = (Func<Person, bool>)GetType().GetMethod(method.ToString()).Invoke(this, [value]);
        var solution = TestData.People.Where(solutionMethod);
        Assert.That(people, Is.EquivalentTo(solution));
    }

    public static Func<Person, bool> GreaterThan(double value) => x => x.Salary > value;

    public static Func<Person, bool> GreaterThanOrEqual(double value) => x => x.Salary >= value;

    public static Func<Person, bool> LessThan(double value) => x => x.Salary < value;

    public static Func<Person, bool> LessThanOrEqual(double value) => x => x.Salary <= value;
}